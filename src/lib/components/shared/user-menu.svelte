...
<div class="relative inline-block">
	<button
		...
		class="relative ml-5 text-center text-gray-700 transition dark:text-white"
	>
	...
	<div
		...
		class="{isUserMenuOpen
			? 'block'
			: 'hidden'}  absolute right-0 z-20 mt-2 w-56 origin-top-right overflow-hidden rounded-md bg-white py-2 shadow-xl dark:bg-gray-800"
	>
		{#if $user}
			<a
				href="#"
			...
			>
				...
				/>
				<div class="mx-1">
					<h1 class="text-sm font-semibold capitalize text-gray-700 dark:text-gray-200">
						{$user.name.firstname}
						{$user.name.lastname}
					</h1>
		...
		<form method="POST" action="/logout">
			<button
				type="submit"
				class="flex w-full transform items-center p-3 text-sm capitalize text-gray-600 transition-colors duration-300 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white"
			>
				<svg
					class="mx-1 h-5 w-5"
					viewBox="0 0 24 24"
